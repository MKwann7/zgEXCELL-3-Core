...
class ListModuleAppsAdminWidget extends VueComponentEntityList
{
    ...
    public function __construct(array $components = [])
    {
        ...
        $displayColumns = array_merge($displayColumns, ["user_name", "on_card", "on_page", "created_on", "last_updated"]);

        $defaultEntity = (new AppInstanceRelModel())
            ->setDefaultSortColumn("app_instance_rel_id", "DESC")
            ->setDisplayColumns($displayColumns)
            ->setFilterColumns(["id","display_name","user_name","status"])
            ->setRenderColumns(["id", "platform", "user_id", "user_name", "on_card", "on_page",  "status", "created_on", "last_updated", "sys_row_id",]);
